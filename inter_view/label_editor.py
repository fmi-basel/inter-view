...
class OrthoView():
    ...
    def __init__(self, config, **kwargs):
        ...
        span_config = {'line_dash':'dashed',
                       'line_width':2,
                       'line_color':'white',
                       'line_alpha':1.0}
                       
        self.top_vspan = Span(dimension='height', location=self.right.slice, **span_config)
        self.top_hspan = Span(dimension='width', location=self.front.slice, **span_config)
        self.right_vspan = Span(dimension='height', location=self.top.slice/self.right.xy_ratio, **span_config)
        self.right_hspan = Span(dimension='width', location=self.front.slice, **span_config)
        self.front_vspan = Span(dimension='height', location=self.right.slice, **span_config)
        self.front_hspan = Span(dimension='width', location=self.top.slice/self.right.xy_ratio, **span_config)
        ...
        for channel in config.keys():
            self.top.renderers[channel].data_source.on_change('data', self.update_image(channel,'top'))
            self.right.renderers[channel].data_source.on_change('data', self.update_image(channel,'right'))
            self.front.renderers[channel].data_source.on_change('data', self.update_image(channel,'front'))
    ...
    def set_images(self, images):
        ...
        self.right.plot_width = int(self.right.plot_height * self.right.width/self.right.height)
        self.right.update_figure_geometry()
        
        self.front.plot_width = self.top.plot_width
        self.front.plot_height = int(self.front.plot_width * self.front.height/self.front.width)
    ...
    def right_tap_callback(self, event):
        ...
        if self.button_toggle_nav.active:
            self.top.slider.value = int(round(event.x*self.right.xy_ratio-0.5))
            self.front.slider.value = int(round(event.y-0.5))
    ...
    def front_tap_callback(self, event):
        ...
        if self.button_toggle_nav.active:
            self.right.slider.value = int(round(event.x-0.5))
            self.top.slider.value = int(round(event.y/self.front.xy_ratio-0.5))
    ...
    def plot(self):
        ...
        self.layout = gridplot([[self.top.layout, self.right.layout],
                             [self.front.layout, self.controls]], merge_tools=False)
...
class ImageStackWithSlider():
    ...
    def __init__(self, config, sampling=1, axis=0, **kwargs):
        ...
        for key,val in config.items():
            ...
            npimg = val.get('npimage', np.zeros((2,16,16), dtype=np.int16)-1)
            npimg = np.transpose(npimg, self.axis_mapping[axis])
            npimg = npimg[:,::-1] # flip x axis to orient image following std convention (top,left) = (0,0)
            self.data[key] = {str(z):[npimg[z]] for z in range(npimg.shape[0])}
        ...
        self.y_range = Range1d(start=self.height, end=0, bounds=(0, self.height))
        
        self.slider = Slider(title='axis({})'.format(axis), start=0, end=self.n_slices-1, value=self.slice, step=1)
    ...
    def plot(self, config):
        ...
        for key,val in config.items():
            ...
            self.renderers[key] = self.p.image(image=str(self.slice),
            ...
            self.renderers[key].glyph.color_mapper.high = val.get('map_high', len(val.get('palette',grey(256)) ))
    ...
    def zoom_callback(self, attr, old, new):
        ...
        if self.width > self.height:
            self.draw_r.glyph.line_width = self.toolsize*(self.p.plot_width)/(self.p.x_range.end-self.p.x_range.start)
        else: 
            self.draw_r.glyph.line_width = self.toolsize*(self.p.plot_height)/(self.p.y_range.start-self.p.y_range.end)
    ...
    def set_drawing_color(self, color):
        ...
        palette = self.renderers[self.drawing_channel].glyph.color_mapper.palette
    ...
    def add_drawing_tools(self, drawing_channel):
        ...
        self.draw_r = self.p.multi_line('xs', 'ys', source=self.draw_source, line_cap='round', line_join='round', line_width=1, line_alpha=self.get_drawing_alpha()) 
        self.set_drawing_color(-1)
        self.set_toolsize(3)
        self.freehand_draw = FreehandDrawTool(renderers=[self.draw_r], num_objects=1)
    ...
    def draw_callback(self, attr, old, new):
        ...
        if new['xs']: # check that the callback is not triggered by deletion (i.e. itself)
            ...
            cv.polylines(npimg[self.slice,::-1],[pts],False,(self.drawing_color),self.toolsize, cv.LINE_4)
            # update datasource
